...
exports.login_form = (req,res,next)=>{
    ...
    if(!is_error || (login_user.username != "" && login_user.password == "") || (login_user.username != "" && login_user.password == ""))
    {
        user_model.get_a_user_by_email(login_user.email, login_user.username)
        .then((selected_user)=>{
            
            console.log(`SELECTED USER IS ${selected_user}`)
            ...
            {
                errors.username = "Sorry! That username or email does not exist!"
                errors.password = "";
