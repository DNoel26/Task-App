...
const user_model = 
{
        ...
        return new Promise((resolve,reject)=>{
            ...
            .then(([rows,fields])=>{
                ...
                if(rows.length > 0)
                {
                    ...
                    selected_user.is_active = rows[0].is_active;
                    ...
                    console.log(`The user for "Get User By Email" is ${selected_user.first_name} - user_MDL.js`);
    ...
    upload_photo(img_path, user_id)
    {
        return new Promise((resolve,reject)=>{

            this.SQL = `UPDATE user SET img_path = ? WHERE user_id = ?`;
            db.connection.query(this.SQL,[img_path, user_id])
            .then(()=>{
                
                require.session.session_user.img_path = new
                resolve();                        
            })
            .catch(err=>reject(err));
        }) 
